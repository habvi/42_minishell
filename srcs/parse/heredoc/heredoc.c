...
static t_result	execute_heredoc_all(t_deque *redirect_list, bool is_interactive)
{
	...
	while (redirect_node)
	{
		...
		heredoc_result = execute_heredoc_each(redirect, is_interactive);
...
t_result	execute_heredoc(t_ast *ast_node, bool is_interactive)
{
	...
	if (!ast_node)
		return (SUCCESS);
	left_result = execute_heredoc(ast_node->left, is_interactive);
	if (left_result == BREAK || left_result == PROCESS_ERROR)
		return (left_result);
	right_result = execute_heredoc(ast_node->right, is_interactive);
	...
	heredoc_result = execute_heredoc_all(\
								ast_node->redirect_list, is_interactive);
