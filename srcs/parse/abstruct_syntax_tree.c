									...
									t_context *context)
{
	...
	left_node = create_command_list_node(token_node, status, context);
	if (!left_node)
		return (NULL);
	while (*token_node && is_token_kind_and_or_from_node(*token_node))
	{
		...
		*token_node = (*token_node)->next;
		right_node = create_command_list_node(token_node, status, context);
									...
									t_context *context)
{
	...
	left_node = create_command_or_subshell_node(token_node, status, context);
	if (!left_node)
		return (NULL);
	while (*token_node && is_token_kind_pipe_from_node(*token_node))
	{
		...
		kind = convert_kind_token_to_node(*token_node); // |
		*token_node = (*token_node)->next;
		right_node = create_command_or_subshell_node(\
												token_node, status, context);
		left_node = new_ast_node(kind, left_node, right_node);
		set_parent_of_children_node(&left_node);
	}
	return (left_node);
	...
	if (is_token_kind_paren_left_as_ast_node(*token_node))
	{
		*token_node = (*token_node)->next;
		if (is_token_kind_paren_right_as_ast_node(*token_node))
			return (NULL);
		ast_node = create_operator_list_node(token_node, status, context);
	}
	if (is_token_kind_paren_right_as_ast_node(*token_node))
	{
		*token_node = (*token_node)->next;
											...
											t_context *context)
{
	...
	if (is_token_kind_command_as_ast_node(*token_node))
		...
		ast_node = create_subshell_node(token_node, status, context);
	if (!ast_node)
		*status = SYNTAX_ERROR;
	return (ast_node);
}
